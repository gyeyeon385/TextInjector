...
**[下載 v0.1.0 預覽版](https://github.com/gyeyeon385/TextInjector/releases/tag/v0.1.0)** · [安裝說明](docs/INSTALL.md) · [驗證紀錄](docs/VALIDATION.md)

原生 macOS SwiftUI 工具，以 CoreGraphics Unicode 鍵盤事件將文字送到 Safari。目前為第一個 Sprint 的技術驗證版，尚未完成 v1.0。
...
- SwiftUI 文字編輯器、測試文字、字數、清除、固定輸入延遲與 Safari 切換延遲。
...
- 只支援單行文字。換行、Tab、Backspace、Escape 等控制字元會在送出任何事件前被拒絕，避免本階段測試觸發表單提交。
...
Fixtures/safari-input.html    Safari 本地驗收頁
...
docs/VALIDATION.md            實際驗證紀錄與後續檢核
```
